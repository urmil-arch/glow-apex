      ...
      <div className="grid lg:grid-cols-2 gap-10 items-center">
        ...
        <motion.div className="flex flex-col gap-2" initial="hidden" animate={isVisible ? "visible" : "hidden"} variants={fadeIn} transition={{ duration: 0.5 }}>
          ...
          <motion.p className="text-lg text-gray-600 mb-4" variants={fadeIn} transition={{ delay: 0.3 }}>
            Give wings to your YouTube videos with high quality YouTube likes that no YouTube algorithm can suspect. Because, they are from real people! Experience the doping effect of the right audience with a reasonable price with BuyRealViews.
          </motion.p>
          ...
          <motion.div className="mt-8 flex items-center gap-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.9 }}>
            ...
            <p className="text-sm text-gray-600"><span className="font-bold text-black">950+</span> customers bought this in the last 24 hours</p>
          </motion.div>
        </motion.div>

        {/* Form Side */}
        <DynamicPackageSelector serviceType="youtube_likes" categoryName="YouTube Likes" title="Buy Youtube Likes" />
      </div>
    </section>
  );
};
