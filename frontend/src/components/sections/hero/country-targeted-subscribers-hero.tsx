...
const CountryTargetedSubscribersHeroSection: React.FC = () => {
  ...
  const [totalPrice, setTotalPrice] = useState<string>("9.99");
  const [convertedPrice, setConvertedPrice] = useState<string>("9.99");
  ...
  const packages: PackagesData = {
    usa: {
      name: "USA Subscribers",
      quantities: [
        { amount: 100, price: "9.99", discount: 0 },
        { amount: 250, price: "19.99", discount: 5 },
        { amount: 500, price: "34.99", discount: 10 },
        { amount: 1000, price: "59.99", discount: 15 },
      ],
    },
    uk: {
      name: "UK Subscribers",
      quantities: [
        { amount: 100, price: "10.99", discount: 0 },
        { amount: 250, price: "22.99", discount: 5 },
        { amount: 500, price: "38.99", discount: 10 },
        { amount: 1000, price: "64.99", discount: 15 },
      ],
    },
    india: {
      name: "India Subscribers",
      quantities: [
        { amount: 100, price: "8.99", discount: 0 },
        { amount: 250, price: "17.99", discount: 5 },
        { amount: 500, price: "29.99", discount: 10 },
        { amount: 1000, price: "49.99", discount: 15 },
      ],
    },
  };
    ...
    <section className="container mx-auto px-4 pb-12 lg:pb-20 pt-44">
      <div className="grid lg:grid-cols-2 gap-10 items-center">
        ...
        >
          ...
          >
            Grow your YouTube channel strategically with real, relevant
            subscribers from specific countries. Target viewers more likely to
            engage with your content and convert into loyal fans or customers.
            Perfect for creators, brands, and businesses focused on regional
            growth.
          </motion.p>
          ...
          >
            ...
            <p className="text-sm text-gray-600">
              <span className="font-bold text-black">800+</span> marketers and
              creators use our geo-targeted services
            </p>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          <motion.div className="text-center relative z-10" variants={fadeIn}>
            <h2 className="text-3xl font-bold bg-gradient-to-r from-teal-400 to-emerald-500 bg-clip-text text-transparent">
              Buy Country Targeted Subscribers
            </h2>
            <p className="text-lg mt-2 text-gray-600">
              Select your target country and subscriber package
            </p>
          </motion.div>

          <motion.div
            className="flex items-center justify-center gap-2 w-full max-w-md z-10 flex-wrap"
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.3 }}
          >
            <motion.button
              className={`min-w-[110px] text-lg py-2 px-4 rounded-lg font-medium transition-all ${
                selectedPackage === "usa"
                  ? "bg-gradient-to-r from-teal-400 to-emerald-500 text-white shadow-md"
                  : "border border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
              }`}
              onClick={(): void => setSelectedPackage("usa")}
              whileHover={{ y: -2 }}
              whileTap={{ y: 0 }}
            >
              USA
            ...
            <motion.button
              ...
              onClick={(): void => setSelectedPackage("uk")}
              whileHover={{ y: -2 }}
              whileTap={{ y: 0 }}
            >
              UK
            ...
            <motion.button
              className={`min-w-[110px] text-lg py-2 px-4 rounded-lg font-medium transition-all ${
                selectedPackage === "india"
                  ? "bg-gradient-to-r from-teal-400 to-emerald-500 text-white shadow-md"
                  : "border border-gray-300 bg-white text-gray-700 hover:bg-gray-50"
              }`}
              onClick={(): void => setSelectedPackage("india")}
              whileHover={{ y: -2 }}
              whileTap={{ y: 0 }}
            >
              India
