...
[2026-06-18 13:00] | modify | backend/app/user_management/repositories/user_repository.py | admin_list_users, admin_get_stats, admin_export_users — added role:"user" base filter so staff and admin accounts are excluded from the admin users list, stats cards, and exports.
[2026-06-18 13:00] | modify | frontend/src/pages/admin/users/UsersPage.tsx | Added selectedIds (Set<string>) state, checkbox column with header select-all (indeterminate state supported), per-row teal highlight on selection. Export buttons skip the date modal and export only selected rows client-side when any are checked; fall back to backend export with date filter when nothing is selected. Selection is cleared on filter/search/page-size changes.

[2026-06-18 13:30] | modify | backend/app/user_management/repositories/user_repository.py | Restructured admin_list_users role filtering: filter_by='staff' now sets role!=user without conflicting with the role=user base guard; all other filter_by values (all/verified/unverified/suspended) still scope to role=user.
[2026-06-18 13:30] | modify | frontend/src/pages/admin/staff/StaffPage.tsx | Full rewrite. MoreVertical actions dropdown per row (skipped for self) with: Edit Details (full_name, username), Change Role (role + extra permissions), Set Password, Sign-in History, Suspend/Unsuspend. Each action opens a dedicated modal. Added Created column. ActiveModal discriminated union drives which modal renders.

[2026-06-18 14:00] | add | frontend/src/glowapex/pages/Home/index.tsx | Added PricingCalculatorSection between WhatWeDoSection and ServicesSection. Two range sliders: Keywords (1–100,000, step 100, $0.05/keyword) and Pages (1–1,000, step 1, $2.00/page). Live price breakdown card shows per-line totals and grand total. Get Started CTA links to /contact. Prices stored as module-level constants PRICE_PER_KEYWORD and PRICE_PER_PAGE for easy future replacement with backend/context values.
