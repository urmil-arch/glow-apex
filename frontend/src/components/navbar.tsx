...
const Navbar = () => {
  ...
  // Load the currency from localStorage on initial render
  useEffect(() => {
  ...
  function renderMenuItems(menuItem: any) {
    ...
    switch (menuItem.type) {
      ...
      case "button":
        return (
          // <li key={menuItem.id} className="px-3 py-2 bg-emerald-500 rounded-full">
          //   <Link
          //     className="nav-link capitalize !text-base"
          //     to={menuItem.href}
          //   >
          <button
            ...
            className="w-fit flex items-center justify-center py-1 px-6 rounded-full text-white font-medium transition-all bg-emerald-700 cursor-pointer"
          >
            {menuItem.title}
          </button>
          // </Link>
  ...
  return (
    <>
      <header
        className={`
        fixed top-0 right-0 left-0 w-full z-40 
      ...
      >
        <div className="container flex items-center justify-between">
          <Link className="py-7" to={"/"}>
            <h1 className="text-xl font-bold"><img src="/web-app-manifest-192x192-removebg-preview.png" alt="Logo" width={50} height={50} /></h1>
          </Link>
