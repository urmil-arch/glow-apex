...
import { BarChart2, Bell, CreditCard, LogOut, Menu, Package, User, X } from "lucide-react";
      ...
      <aside className={`bg-white shadow-lg fixed h-screen z-30 transition-all duration-300 transform ${isSidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} ${isMobile ? "w-64" : isSidebarOpen ? "w-64" : "w-20 md:w-20"}`}>
        <div className="p-4 flex items-center justify-between border-b">
          <div className={`${isSidebarOpen || !isMobile ? "block" : "hidden"}`}>
            <h1 className="text-xl font-bold">GLOW APEX</h1>
          </div>
            ...
            <div className="relative w-10 h-10 rounded-full overflow-hidden">
              {user.avatar ? (
                ...
                <div className="bg-emerald-100 w-full h-full flex items-center justify-center">
                  <span className="text-emerald-700 font-medium text-lg">{user.name.charAt(0)}</span>
                </div>
              )}
            </div>
            {isSidebarOpen && (
        ...
        <main className="p-4 md:p-6 flex-grow">
          <div className="bg-white rounded-xl shadow-md p-4 md:p-6 mb-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h2 className="text-lg md:text-xl font-semibold mb-2">Welcome back, {user.name}!</h2>
                <p className="text-gray-600 text-sm md:text-base">Here's a summary of your account activity and orders.</p>
              </div>
              <div className="mt-4 sm:mt-0">
                <button className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg px-4 py-2 font-medium transition-colors">
                  Place New Order
                </button>
              </div>
            </div>
