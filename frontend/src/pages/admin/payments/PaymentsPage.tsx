...
function mapOrderStatus(raw: string): { label: string; cls: string } {
  ...
  if (s === 'failed')          return { label: 'Error: Order Failed', cls: 'bg-red-100 text-red-700' };
  if (s === 'provider error')  return { label: 'Error: Provider',  cls: 'bg-red-100 text-red-700' };
  if (s.includes('error'))     return { label: `Error: ${raw}`,    cls: 'bg-red-100 text-red-700' };
  if (!raw)                    return { label: '—',                cls: 'bg-gray-100 text-gray-400' };
  return                              { label: raw,                cls: 'bg-gray-100 text-gray-600' };
}

const payStatusCls = (s: string): string => {
  const l = s.toLowerCase();
  if (l === 'completed' || l === 'paid') return 'bg-green-100 text-green-700';
...
const shortOrderId = (id: string) => id ? `#${id.slice(-8).toUpperCase()}` : '—';
...
const AddPaymentModal = ({ onClose, onSaved }: AddPaymentModalProps) => {
  const [form, setForm] = useState({
    user_search: '', user_id: '', amount: '',
    type: 'credit' as 'credit' | 'debit', memo: '', status: 'Completed',
  });
  ...
  const selectUser = (u: UserOption) => {
    setForm(f => ({ ...f, user_id: u.id, user_search: `${u.username} (${u.email})` }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(API_ENDPOINTS.ADMIN_PAYMENTS, {
        user_id: form.user_id, amount: Number(form.amount),
        method: 'Manual', type: form.type, memo: form.memo.trim(), status: form.status,
      });
      onSaved();
    } catch (err: unknown) {
      if (axios.isAxiosError(err)) setError(err.response?.data?.detail ?? 'Failed to add payment.');
      else setError('Failed to add payment.');
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={e => { setForm(f => ({ ...f, user_search: e.target.value, user_id: '' })); searchUsers(e.target.value); }}
              ...
              {(['credit', 'debit'] as const).map(t => (
                <button key={t} type="button" onClick={() => setForm(f => ({ ...f, type: t }))}
                  className={`flex-1 py-2 text-sm font-medium rounded-lg border transition-colors capitalize ${
                    form.type === t
                      ? t === 'credit' ? 'bg-green-600 text-white border-green-600' : 'bg-red-500 text-white border-red-500'
          ...
          <div className="flex justify-end gap-2 pt-1">
            <button type="button" onClick={onClose}
              className="px-4 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors">Cancel</button>
...
const AdminPaymentsPage = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault(); setSearch(searchInput.trim()); setPage(1);
  };

  const handleExport = () => {
    if (payments.length === 0) return;
    const headers = 'Order ID,User,Email,Balance,Amount,Method,Type,Pay Status,Order Status,Service,Memo,Created';
    const rows = payments.map(p => [
      p.order_id, p.user_username, p.user_email, p.user_balance.toFixed(2),
      p.amount.toFixed(2), p.method, p.type, p.status, p.order_status,
      `"${(p.category_name || p.service_name).replace(/"/g, '""')}"`,
      `"${p.memo.replace(/"/g, '""')}"`, p.created_at,
    ].join(','));
    const blob = new Blob([[headers, ...rows].join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `payments-page-${page}.csv`; a.click();
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    try { await api.delete(`${API_ENDPOINTS.ADMIN_PAYMENTS}/${id}`); fetchPayments(); }
    ...
    <div>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        <form onSubmit={handleSearch} className="flex gap-2 ml-auto">
          ...
          <button type="submit" className="px-3 py-2 bg-teal-600 text-white text-sm rounded-lg hover:bg-teal-700 transition-colors">Search</button>
          {search && (
            <button type="button" onClick={() => { setSearch(''); setSearchInput(''); setPage(1); }}
              className="px-3 py-2 text-sm text-gray-600 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">Clear</button>
          )}
        </form>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20 text-gray-400 gap-2">
            <CreditCard className="w-10 h-10 text-gray-300" /><p className="text-sm">No payments found</p>
          </div>
        ) : (
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {payments.map((p) => {
                    const { label: osLabel, cls: osCls } = mapOrderStatus(p.order_status);
                    const serviceName = p.category_name || p.service_name || p.memo || '—';
                    return (
                      <tr key={p.id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                        {/* Order ID */}
                        <td className="px-4 py-3 font-mono text-xs text-gray-600 whitespace-nowrap">
                          {shortOrderId(p.order_id)}
                        </td>
                        {/* User */}
                        ...
                        {/* Pay Status */}
                        <td className="px-4 py-3">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${payStatusCls(p.status)}`}>
                            {p.status}
                          </span>
                        </td>
                        {/* Order Status */}
                        <td className="px-4 py-3 hidden md:table-cell">
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${osCls}`}>
                            {osLabel}
                          </span>
                        </td>
                        {/* Service */}
                        <td className="px-4 py-3 hidden lg:table-cell max-w-[160px]">
                          <span className="text-xs text-gray-700 block truncate" title={serviceName}>
                            {serviceName}
                          </span>
                          {p.quantity > 0 && (
                            <span className="text-xs text-gray-400">Qty: {p.quantity.toLocaleString()}</span>
                          )}
                        </td>
                        {/* Link */}
                        <td className="px-4 py-3 hidden xl:table-cell max-w-[180px]">
                          {p.order_link ? (
                            <a href={p.order_link} target="_blank" rel="noopener noreferrer"
                        ...
                        {/* Memo */}
                        <td className="px-4 py-3 text-xs text-gray-500 hidden lg:table-cell max-w-[160px] truncate">
                          {p.memo || '—'}
                        </td>
                        {/* Created */}
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
              <p className="text-xs text-gray-400">{total} payment{total !== 1 ? 's' : ''} total</p>
              <div className="flex items-center gap-1">
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="px-2 py-1 text-xs text-gray-600 border border-gray-200 rounded hover:bg-gray-50 disabled:opacity-40">«</button>
                  ...
                  return (
                    <button key={pg} onClick={() => setPage(pg)}
                      className={`px-2.5 py-1 text-xs rounded border transition-colors ${
                        page === pg ? 'bg-teal-600 text-white border-teal-600' : 'text-gray-600 border-gray-200 hover:bg-gray-50'
                      }`}>{pg}</button>
                  );
                })}
